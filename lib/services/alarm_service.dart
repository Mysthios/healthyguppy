  ...
  static Future<void> scheduleActiveAlarms() async {
    try {
      ...
      for (var notif in activeNotifs) {
        ...
        // Schedule notification - PERBAIKAN: gunakan static method
        await NotificationService.scheduleNotification(
          id: id,
          title: notif.judul,
          body: notif.isi,
...
void alarmCallback() async {
  try {
    // PERBAIKAN: Dapatkan ID alarm yang benar dari context atau parameter
    // Untuk sementara, kita akan mencoba mencari data yang tersimpan
    final prefs = await SharedPreferences.getInstance();
    
    // Cari semua alarm yang tersimpan dan trigger notifikasi
    final keys = prefs.getKeys();
    final titleKeys = keys.where((key) => key.startsWith('alarm_title_'));
    
    for (String titleKey in titleKeys) {
      final id = titleKey.replaceFirst('alarm_title_', '');
      final title = prefs.getString(titleKey) ?? 'Alarm!';
      final body = prefs.getString('alarm_body_$id') ?? 'Waktunya memberi makan ikan!';
      
      // Trigger notifikasi - PERBAIKAN: gunakan static method
      ...
      debugPrint('🔔 Alarm triggered: $title - $body');
      break; // Hanya trigger satu notifikasi pertama untuk menghindari spam
    }
  } catch (e) {
    debugPrint('❌ Error in alarm callback: $e');
  }
